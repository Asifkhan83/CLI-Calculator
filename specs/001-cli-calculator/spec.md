...
- **Success**: Displays the integer result on a single line
- **Error**: Displays an error message starting with `Error:` followed by a description
...
1. **Given** the calculator is ready, **When** the user enters `10 / 0`, **Then** the system displays a clear error message indicating division by zero is not allowed
2. **Given** the calculator is ready, **When** the user enters `5 + 10 / 0`, **Then** the system displays a clear error message (division by zero detected during evaluation)
...
1. **Given** the calculator is ready, **When** the user enters `abc`, **Then** the system displays an error message indicating invalid input
...
5. **Given** the calculator is ready, **When** the user enters an empty input, **Then** the system displays an error message indicating input is required
...
- What happens when the user enters negative numbers? (Assumption: Supported, e.g., `-5 + 3` returns `-2`)
- What happens when the user enters expressions with extra whitespace? (Assumption: Whitespace is trimmed/ignored, `5  +  3` works like `5 + 3`)
...
| `-5 + 3` | Negative operand at start | `-2` |
| `5 * -3` | Negative operand in multiplication | `-15` |
| `10 / -2` | Negative divisor | `-5` |
| `2 + 3 * 4 - 1` | Multiple operators | `13` |
| `100 / 10 / 2` | Chained same-precedence | `5` |
...
| `+ 5` | Malformed expression | Missing operand before operator |
...
3. **Whitespace**: Optional; ignored during evaluation
4. **Negative numbers**: Leading minus directly attached to number (e.g., `-5`)
5. **Expression structure**: Must alternate between operands and operators

## Error Handling Specification

### Error Message Types

| Error Type | Trigger Condition | Example Message |
|------------|-------------------|-----------------|
| Division by Zero | Right operand of `/` is `0` | `Error: Division by zero is not allowed` |
| Invalid Operand | Non-integer value (letters, decimals, symbols) | `Error: Invalid operand 'abc'. Only integers are allowed.` |
| Invalid Operator | Unsupported operator symbol | `Error: Unsupported operator '^'. Supported operators: +, -, *, /` |
| Malformed Expression | Incomplete or syntactically invalid | `Error: Incomplete expression. Expected operand after '+'.` |
| Empty Input | No expression provided | `Error: No input provided. Please enter a mathematical expression.` |
...
- **FR-005**: System MUST perform integer division (truncating toward zero for non-whole results)
...
- **FR-008**: System MUST display the calculated result to the user after successful evaluation
- **FR-009**: System MUST handle negative integers in expressions (e.g., `-5 + 3`)
...
- **FR-012**: Error messages MUST be specific and include guidance on expected input format
...
- **SC-002**: All calculations with multiple operators return mathematically correct results following BODMAS precedence
- **SC-003**: 100% of division-by-zero attempts result in a clear, user-friendly error message (no crashes or unclear errors)
- **SC-004**: 100% of invalid input attempts result in a clear, user-friendly error message indicating the problem
- **SC-005**: Users can complete a basic calculation (enter expression, see result) in under 2 seconds
...
- Integer division truncates toward zero (e.g., `7 / 2 = 3`, `-7 / 2 = -3`)
- Whitespace in expressions is ignored/trimmed
- Negative numbers are supported using the minus sign (e.g., `-5`)
